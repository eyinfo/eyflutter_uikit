...
class _RefreshFooterControllerState extends State<RefreshFooterController> {
  ...
  Widget build(BuildContext context) {
    // 是否到达触发加载距离
    overTriggerDistance = widget.mode != LoadMode.inactive && widget.pulledExtent >= widget.loadTriggerPullDistance;
    _isShowIndicator = (widget.mode == LoadMode.done || widget.mode == LoadMode.loaded) ? false : true;
    _footerHeight = widget.loadIndicatorExtent > widget.pulledExtent ? widget.loadIndicatorExtent : widget.pulledExtent;
    return Container(
      ...
      children: [
        ...
        Offstage(
          ...
          child: Text(
            ...
            style: TextStyle(
                letterSpacing: 0.82,
                color: Color(RefreshInitManager.instance.loadHandler().loadTextColor() ?? 0xff666666),
